...
public class TileMap {
    ...
    public TileMap(String mapFilePath) {
        ...
        if (gateLayer != null) {
            ...
            // Retrieve the gate objects' properties and mark the corresponding tiles as boundary tiles
            for (MapObject object : gateLayer.getObjects()) {
                if (object instanceof RectangleMapObject) {
                    ...
                    boolean isBoundary = false;
                    if (object.getProperties().containsKey("boundary")) {
                        isBoundary = object.getProperties().get("boundary", Boolean.class);
                    }
    ...
    public boolean isBoundary(int x, int y) {
        ...
        return boundaryTiles[x][y] || gateTiles[x][y]; // Include gateTiles in the condition
    }
    ...
    public void updateGateBoundary(MapObject gateObject, boolean gateBoundary) {
        ...
        if (gateObject instanceof RectangleMapObject) {
            ...
            // Update the gate object in the gate layer
            for (MapObject object : gateLayer.getObjects()) {
                if (object.getName().equals(gateObject.getName())) {
                    object.getProperties().put("boundary", gateBoundary);
                    break;
                }
            }
        }

        // Retrieve the gate object's boundary property from the tiled map object
        boolean gateBoundaryUpdate = false;
        for (MapObject object : gateLayer.getObjects()) {
            if (object.getName().equals(gateObject.getName())) {
                gateBoundaryUpdate = object.getProperties().get("boundary", Boolean.class);
                break;
            }
        }
